...
if ! docker ps >/dev/null ; then
    ...
    if echo "$existing_context_list" | grep -q "colima"; then
...
docker build -t openwrt-lom:latest . || { errormsg "failed to build Docker image"; exit 1; }
docker run --name openwrt-lom -dt openwrt-lom bash || { errormsg "failed to run Docker container"; exit 1; }